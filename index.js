...
switch(action) {
  case "test":
    ...
    fs.renameSync("_locales/" + manifest.default_locale, "_locales/" + testName);
    fs.renameSync("_locales/" + testLocale, "_locales/" + manifest.default_locale);
    console.log("Done! When finish testing call:\nchrome-locale-verify testend " + testLocale);
  break;
  case "testend":
    var testLocale = argv._[1];
    var testName = "_test_" + argv._[1];
    if(!fs.existsSync("_locales/" + testName)) {
      end("Test not exists", "red");
    }
    fs.renameSync("_locales/" + manifest.default_locale, "_locales/" + testLocale);
    fs.renameSync("_locales/" + testName, "_locales/" + manifest.default_locale);
  ...
  case "check":
    ...
    var defaultLocaleMessages = JSON.parse(
      fs.readFileSync("_locales/" + manifest.default_locale + "/messages.json", "utf8")
    );
    var defaultLocaleMessagesNames = Object.keys(defaultLocaleMessages);
    var localesFolders = glob.sync("_locales/*");
